...
import { getResourceDescriptor } from './getResourceDescriptor.js';
import { mergeIntoTemplatesCategory } from './mergeIntoTemplatesCategory.js';
...
export class BaseAppendMenuProvider {
  ...
  _register() {
    this._popupMenu.registerProvider('bpmn-append', LOWER_PRIORITY, this);
  };
  ...
  getPopupMenuEntries(element) {
    ...
    const resourceEntries = this._buildResourceEntries(element);

    return (entries) => mergeIntoTemplatesCategory(entries, resourceEntries, {
      idPrefix: 'append',
      translate: this._translate
    });
  };

  _buildResourceEntries(element) {
    const {
      resourceType,
      groupName,
      createElement,
    ...
    resources.forEach((resource, index) => {
      ...
      const resourceDescriptor = getResourceDescriptor(resource);
